...
class MultiSignal(gym.Env):
    ...
    def create_additional_file(self, filename):
        # Create additional xml file for output lane data and edge data
        save_path = os.path.join(self.log_dir, self.connection_name, 'lanedata')
        # save_path = os.path.join(self.log_dir, self.connection_name, 'edgedata')
        if not os.path.exists(save_path):
            os.makedirs(save_path)
    ...
    def reset(self):
        ...
        additional_file = os.path.join(self.log_dir, self.connection_name, 'additional.xml')
        self.create_additional_file(additional_file)
    ...
    def save_metrics(self):
        save_path = log = os.path.join(self.log_dir, self.connection_name, 'metrics')
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        log = os.path.join(save_path, 'metrics_' + str(self.run) + '.csv')
        print('saving', log)
