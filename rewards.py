...
def pressure_rand(signals):
    rewards = dict()
    for signal_id in signals:
        queue_length = 0
        for lane in signals[signal_id].lanes:
            
            ql = signals[signal_id].full_observation[lane]['queue']
            random_change = random.randint(-1, 1)
            ql += random_change
            ql = max(queue_length, 0)
        ...
        for lane in signals[signal_id].outbound_lanes:
            dwn_signal = signals[signal_id].out_lane_to_signalid[lane]
            if dwn_signal in signals[signal_id].signals:
                ql = signals[signal_id].signals[dwn_signal].full_observation[lane]['queue']
                random_change = random.randint(-1, 1)
                ql += random_change
                ql = max(queue_length, 0)
                queue_length -= ql
