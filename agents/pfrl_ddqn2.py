...
class IDDQN2(IndependentAgent):
    def __init__(self, config, obs_act, map_name, thread_number):
        ...
        for key in obs_act:
            ...
            model = DuelingDQN(obs_space[0], act_space)

            self.agents[key] = DQNAgent(config, act_space, model)
            if self.config['load']:
                print('LOADING SAVED MODEL FOR EVALUATION')
                self.agents[key].load(os.path.join(self.config['log_dir'],'agent', 'agent_'+key+'.pt'))
                self.agents[key].agent.training = False
                

class DuelingDQN(nn.Module):
    def __init__(self, in_channels, num_actions, output_size=64):
        super(DuelingDQN, self).__init__()
        self.conv = nn.Sequential(
            nn.Conv1d(in_channels, 32, kernel_size=2),
            nn.ReLU(),
            nn.Conv1d(32, 64, kernel_size=2),
            nn.ReLU(),
            nn.AdaptiveAvgPool1d(output_size)  # Ensures the output is always the same size
        )
        self.value_stream = nn.Sequential(
            nn.Linear(64 * output_size, 128),  # 64 filters, output_size from adaptive pooling
            nn.ReLU(),
            nn.Linear(128, 1)
        )
