...
class IDDQN3(IndependentAgent):
    def __init__(self, config, obs_act, map_name, thread_number):
        ...
        for key in obs_act:
            ...
            model = DuelingDQN(obs_space[0], h, w, act_space)
...
class DuelingDQN(nn.Module):
    def __init__(self, in_channels, h, w, num_actions):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(in_channels, 32, kernel_size=(2, 2)),  # Reduced complexity if needed
            nn.ReLU(),
            nn.Flatten(),
            nn.Linear(h * w * 32, 256),  # Increased size of the linear layer
            nn.ReLU(),
            nn.Linear(256, 128),  # Increased size for deeper understanding
            nn.ReLU()
        )
